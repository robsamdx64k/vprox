...
// ----- Pool failover (global within this worker) -----
let _configRef = null;
let _pools = [];               // normalized pool list
let _activePoolIndex = 0;      // current active upstream pool index
let _failureStreak = 0;        // consecutive failures
let _lastSwitchAt = 0;         // ms
let _lastPrimaryOkAt = 0;      // ms

function normalizePools(config) {
    // Backward compatible: allow legacy config.pool
    if (!Array.isArray(config.pools) || config.pools.length === 0) {
        if (config.pool && config.pool.host && config.pool.port) {
            config.pools = [{ name: 'primary', ...config.pool }];
        } else {
            config.pools = [];
        }
    }
    // Ensure each pool has a name
    config.pools = config.pools.map((p, i) => ({
        name: (p && p.name) ? String(p.name) : (i === 0 ? 'primary' : `backup${i}`),
        ...p
    }));

    // Failover defaults
    if (!config.poolFailover) config.poolFailover = {};
    if (!config.poolFailover.strategy) config.poolFailover.strategy = 'priority';
    if (!config.poolFailover.retryMs) config.poolFailover.retryMs = 5000;
    if (!config.poolFailover.maxFailures) config.poolFailover.maxFailures = 3;
    if (!config.poolFailover.autoReturn) config.poolFailover.autoReturn = true;
    if (!config.poolFailover.primaryCheckMs) config.poolFailover.primaryCheckMs = 30000;

    _pools = config.pools;
    if (_activePoolIndex < 0 || _activePoolIndex >= _pools.length) _activePoolIndex = 0;
    config.pool = _pools[_activePoolIndex] || config.pool;
}

function switchActivePool(config, newIndex, reason) {
    if (!_pools.length) return;
    const idx = Math.max(0, Math.min(newIndex, _pools.length - 1));
    if (idx === _activePoolIndex) return;

    const prev = _pools[_activePoolIndex];
    const next = _pools[idx];
    _activePoolIndex = idx;
    config.pool = next;
    _failureStreak = 0;
    _lastSwitchAt = Date.now();
    console.log(`POOL failover: switching to [${idx}] ${next.name || ''} ${next.host}:${next.port} (from ${prev ? prev.host + ':' + prev.port : 'unknown'}) reason=${reason}`);

    // Force miners to reconnect to new pool by closing existing pool sockets.
    // This keeps behavior predictable and avoids mixed-upstream state.
    for (const p of poolConnections.values()) {
        try { p.destroy(); } catch(e) {}
    }
}

function notePoolFailure(config, why) {
    if (!_pools.length) return;
    _failureStreak++;
    const maxFailures = Number(config.poolFailover?.maxFailures || 3);
    if (_failureStreak >= maxFailures) {
        // rotate to next pool (priority order)
        const next = (_activePoolIndex + 1) % _pools.length;
        switchActivePool(config, next, why || 'failure');
    }
}

function startAutoReturnToPrimary(config) {
    if (!config.poolFailover?.autoReturn) return;
    if (!_pools.length) return;
    if (_pools.length < 2) return;

    const primary = _pools[0];
    const checkMs = Number(config.poolFailover?.primaryCheckMs || 30000);

    const tryPrimary = () => {
        if (_activePoolIndex === 0) return; // already on primary

        // Attempt a quick TCP/TLS connect probe.
        const timeoutMs = 3000;
        const onOk = () => {
            _lastPrimaryOkAt = Date.now();
            switchActivePool(config, 0, 'auto-return');
        };
        ...
        try {
            ...
            let sock;
            if (primary.ssl === true) {
                sock = tls.connect({ host: primary.host, port: primary.port, rejectUnauthorized: false }, () => done(true));
            } else {
                sock = net.createConnection({ host: primary.host, port: primary.port }, () => done(true));
            }
    ...
    setInterval(tryPrimary, checkMs);
}
...
exports.createMiningListener = function createMiningListener(config) {
    ...
    // Normalize pool config and enable failover/auto-return if configured.
    normalizePools(config);
    startAutoReturnToPrimary(config);
    
    // Optional periodic miner stats logging
    if (config.statsLogIntervalMs && config.statsLogIntervalMs > 0) {
        setInterval(() => {
            try {
                const snap = 
// Manually switch upstream pool by name or index. This triggers the same behavior as failover
// (closing pool sockets to force miners to re-establish against the new upstream).
exports.manualSwitchPool = function(target) {
    try {
        if (!_configRef) return { ok:false, error:"no_config" };
        if (!_pools || _pools.length === 0) return { ok:false, error:"no_pools" };

        let idx = -1;
        if (target && typeof target.index === "number" && !isNaN(target.index)) {
            idx = Math.max(0, Math.min(parseInt(target.index, 10), _pools.length - 1));
        } else if (target && target.name != null) {
            const name = String(target.name);
            idx = _pools.findIndex(p => (p && p.name) ? String(p.name) === name : false);
            if (idx < 0 && !isNaN(name)) {
                idx = Math.max(0, Math.min(parseInt(name, 10), _pools.length - 1));
            }
        }
        if (idx < 0) return { ok:false, error:"not_found" };

        switchActivePool(_configRef, idx, (target && target.reason) ? target.reason : "manual");
        return { ok:true, activePoolIndex: _activePoolIndex, activePool: _pools[_activePoolIndex] };
    } catch (e) {
        return { ok:false, error:"exception" };
    }
};

exports.getMinerStatsSnapshot();
                ...
                console.log(`STATS miners=${total} submitted=${submitted} accepted=${accepted} rejected=${rejected}`);
            } catch(e) {}
    ...
    let handleNewClient = function(socket) {
        ...
        socket.lastIds = [];
        // Miner tracking (basic stats)
        socket.lastMethods = [];
        socket.lastTimes = [];
        socket.stats = {
            minerId: socket.minerId,
            ip: socket.peerEndpoint,
            ...
            lastSubmitAt: 0,
            lastShareResultAt: 0,
            submitLatencyMsSum: 0,
            submitLatencyCount: 0
        };

        socket.getMsgInfo = function() {
            if (socket.lastIds.length > 0) {
                return {
                    id: socket.lastIds.shift(),
                    method: socket.lastMethods.shift(),
                    ts: socket.lastTimes.shift()
                };
            }
            return { id: 0, method: "", ts: 0 };
        };
        ...
        socket.getMsgId = function() {
            // Backwards-compat: return only the original message id, while keeping tracking arrays aligned
            if (socket.lastIds.length > 0) {
                socket.lastMethods.shift();
                socket.lastTimes.shift();
        ...
        socket.reconnectRequest = function() {
            ...
            if (sinceLastFailedAttempt > 15000 && !socket.pool && waitingForPoolConnection !== true) {
                ...
                // Failover tracking (best-effort)
                pool.on('error', () => { try { notePoolFailure(config, 'error'); } catch(e) {} });
                pool.on('close', () => {
                    try {
                        // Only count as failure if miners were attached.
                        if (pool.miners && pool.miners.size > 0) notePoolFailure(config, 'close');
                    } catch(e) {}
                });
            ...
            while (di > -1) {
                ...
                if (obj.method == 'mining.subscribe') {
                    ...
                    if (!socket.subscribed) {
                        ...
                        socket.stats.subscribedAt = Date.now();
                    ...
                    if (!socket.miner) {
                        let handled = true;
                        // Track stratum worker name (as sent by miner). This is commonly wallet.worker.
                        try {
                            socket.stats.workerName = poolListener.getSafeString(obj.params && obj.params[0] ? obj.params[0] : "");
                        } catch(e) {}
                        ...
                        if (!authObj) {
                            ...
                            if (authObj.params && authObj.params[0].indexOf(socket.miner) == -1) {
                                ...
                                if (reconnectionRequired === true) {
                                    ...
                                    pool.on('error', () => { try { notePoolFailure(config, 'error'); } catch(e) {} });
                                    pool.on('close', () => {
                                        try { if (pool.miners && pool.miners.size > 0) notePoolFailure(config, 'close'); } catch(e) {}
                                    });
                                    pool.setAuthorizeObject(obj);
                                    poolConnections.set(socket.miner, pool);
                                    let poolClosed = function () {
                        ...
                        socket.stats.authorizedAt = Date.now();
                        waitingForPoolConnection = false;
                    ...
                    if (socket.authorized && socket.subscribed) {
                        socket.stats.submitted++;
                        socket.stats.lastSubmitAt = Date.now();
...
// Miner tracking snapshot (safe for JSON output)
exports.getMinerStatsSnapshot = function() {
    const out = { total: 0, active: [], miningWorkers: 0, poolState: null, updatedAt: Date.now() };
    try {
        out.total = miners.size;
        const workers = new Set();
        for (let s of miners.values()) {
            if (!s || !s.stats) { continue; }
            const st = Object.assign({}, s.stats);
            st.uptimeSec = Math.floor((Date.now() - (st.connectedAt || Date.now())) / 1000);
            st.avgSubmitLatencyMs = st.submitLatencyCount > 0
                ? Math.round(st.submitLatencyMsSum / st.submitLatencyCount)
                : 0;
            out.active.push(st);
            if (st.workerName) workers.add(String(st.workerName));
        }
        out.miningWorkers = workers.size;
        out.poolState = {
            activePoolIndex: _activePoolIndex,
            pools: Array.isArray(_pools) ? _pools.map((p,i)=>({name:p.name, host:p.host, port:p.port, ssl: p.ssl===true})) : [],
            activePool: (_pools && _pools[_activePoolIndex]) ? {
                name: _pools[_activePoolIndex].name,
                host: _pools[_activePoolIndex].host,
                port: _pools[_activePoolIndex].port,
                ssl: _pools[_activePoolIndex].ssl === true
            } : null,
            failureStreak: _failureStreak,
            lastSwitchAt: _lastSwitchAt,
            lastPrimaryOkAt: _lastPrimaryOkAt
        };
    } catch(e) {}
    return out;
};
